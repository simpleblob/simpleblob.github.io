...
System 1 examples\\
Detect moving objects, complete familiar phrases, Detect/recognize
random sounds.

\\

System 2 examples\\
Mental 2+ digits multiplication, focus on voice of one person in a
crowded room.
...
diminishes -- with fewer brain regains involved.  (Chunking into
habits?)\\
...
What is effortful?\\
1. Adoption and termination of new task sets. (Never seen nor tried
before)\\
2. Switching from one task to another.\\
Note: both 1. And 2. are mindful tasks.\\
3. Constantly using Working memory to the fullest.
...
uncertain or not thinking much (due to habit or time - to - decisions. )
The effect depends on recent events - time series of short term
memory.\\
...
Familiarity/ease -> system 1 engages\\
= more accepting/gullible

\\

Unfamiliar -> system 2 engages\\
= doubtful/critical
...
System 1 is doing greedy search , therefore good at positive one - sided
affirmation.
...
-  Sets & prototypes - system 1 do well on estimating averages but badly
   on sums. 100 vs 10k People died  = same effect.
